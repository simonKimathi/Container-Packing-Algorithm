package com.packingAlgorithm.Algorithm;

import com.packingAlgorithm.Comparator.BoxAreaComparator;
...
public class PackingAlgorithm {
	...
	public void statPack(ShapeActions shape, List<ShapeActions> aoShapes) {
		...
		int liTotalBoxVolume = ShapeActions.getTotalVolume(aoShapes);
	...
	public int pack(ShapeActions shape, List<ShapeActions> aoShapes) {
		// Optimize algorithm performance by sorting the boxes (area ascending)
		Collections.sort(aoShapes, new BoxAreaComparator());
		Collections.reverse(aoShapes);
		return this.pack(shape, aoShapes, 0, false);
	}

	private int pack(ShapeActions shape, List<ShapeActions> aoShapes,
			int aiContainerNum, boolean abSubContainer) {

		// Step 1: Find a shape that fits into the container
		Iterator<ShapeActions> loIterator = aoShapes.iterator();
		ShapeActions loCurrentShape = null;
		...
		if (FoundFit) {
			...
			List<ShapeActions> loSubContainers = loCurrentShape.breakUp(shape);
			for (ShapeActions loSubContainer : loSubContainers) {
